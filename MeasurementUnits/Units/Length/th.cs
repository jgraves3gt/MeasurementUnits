...
{
    /// <summary>
    /// Thou or thousandth of an inch
    /// </summary>
    public class th : ILength
    {
        public double Value { get; set; }
        public double BaseReference { get { return 0.0254; } }
        ...
        public static th FromLengthUnit<TLength>(TLength source) where TLength : ILength
        {
            var result = new th();
        ...
        public static th ConvertFrom<TLength>(TLength source) where TLength : ILength
        {
            var result = new th();
            var baseValue = source.Value * source.BaseReference;
            result.Value = baseValue / result.BaseReference;
        ...
        public static th ConvertFrom(LengthUnits unit, double value)
        {
            switch (unit)
            {
                default:
