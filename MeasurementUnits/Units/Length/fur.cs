...
namespace MeasurementUnits.Units.Length
{
    ...
    /// Furlong
    /// </summary>
    public class fur : ILength
    {
        public double Value { get; set; }
        public double BaseReference { get { return 201168; } }
        ...
        public static fur FromLengthUnit<TLength>(TLength source) where TLength : ILength
        {
            var result = new fur();
        ...
        public static fur ConvertFrom<TLength>(TLength source) where TLength : ILength
        {
            var result = new fur();
            var baseValue = source.Value * source.BaseReference;
            result.Value = baseValue / result.BaseReference;
        ...
        public static fur ConvertFrom(LengthUnits unit, double value)
        {
            switch (unit)
            {
                default:
