...
namespace MeasurementUnits.Units.Length
{
    ...
    public class cm : ILength
    {
        ...
        public static cm FromLengthUnit<TLength>(TLength source) where TLength : ILength
        {
            var result = new cm();
        ...
        public static cm ConvertFrom<TLength>(TLength source) where TLength : ILength
        {
            var result = new cm();
            var baseValue = source.Value * source.BaseReference;
            result.Value = baseValue / result.BaseReference;
        ...
        public static cm ConvertFrom(LengthUnits unit, double value)
        {
            switch (unit)
            {
                default:
