...
namespace MeasurementUnits.Units.Length
{
    ...
    public class mm : ILength
    {
        ...
        public static mm FromLengthUnit<TLength>(TLength source) where TLength : ILength
        {
            var result = new mm();
        ...
        public static mm ConvertFrom<TLength>(TLength source) where TLength : ILength
        {
            var result = new mm();
            var baseValue = source.Value * source.BaseReference;
            result.Value = baseValue / result.BaseReference;
        ...
        public static mm ConvertFrom(LengthUnits unit, double value)
        {
            switch (unit)
            {
                default:
