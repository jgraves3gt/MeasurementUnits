...
namespace MeasurementUnits.Units.Length
{
    ...
    public class m : ILength
    {
        ...
        public static m FromLengthUnit<TLength>(TLength source) where TLength : ILength
        {
            var result = new m();
        ...
        public static m ConvertFrom<TLength>(TLength source) where TLength : ILength
        {
            var result = new m();
            var baseValue = source.Value * source.BaseReference;
            result.Value = baseValue / result.BaseReference;
        ...
        public static m ConvertFrom(LengthUnits unit, double value)
        {
            switch (unit)
            {
                default:
